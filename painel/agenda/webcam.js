		...
		switch (type) {
			...
			case 'error':
				...
				if (!this.fire_hook('onError', msg)) {
					//alert("Web cam não detectada, favor solicitar suporte ao Guilherme | TI.");
					$("#errowebcam").html("Web cam não detectada, favor solicitar suporte ao TI - Acesse o Help Desk pelo Portal. </br></br>O cadastro pode ser feito normalmente, porém é necessário que futuramente a foto seja cadastrada.");
					$("#btnConfigurarWebCam").css('display', 'none');
					$("#brntirarFoto").css('display', 'none');
					$("#brnRefazerFoto").css('display', 'none');
					$("#webcam_movie").css('display', 'none');
				}
				break;
